...
(function () {
  ...
  let basePositions, positions, velocities;
  ...
  // Compact noise implementation (no dependencies)
  function noise3D(x, y, z) {
    ...
    81,51,145,235,249,14,239,107,49,192,214,31,181,199,106,157,184,84,204,176,115,121,50,45,127,4,150,254,138,
  ...
  function createParticles() {
    ...
    const electricBlue = new THREE.Color(0x4466ff);
    const violet = new THREE.Color(0x7c4dff);
    const white = new THREE.Color(0xeeeeff);
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // Color gradient based on Y-position (top → cyan, middle → blue, bottom → violet)
      const yNorm = (basePositions[i3 + 1] + r) / (2 * r); // 0 to 1
      let color;
      if (yNorm > 0.6) {
  ...
  function onMouseMove(e) {
    ...
    // Project mouse into 3D world on the Z=0 plane
  ...
  // ── Animation loop ─────────────────────────────────────
  let smoothMouse = new THREE.Vector3();
  ...
  function animate() {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // Normalize base position for noise coords
      const nx = bx * 0.008;
      const ny = by * 0.008;
      const nz = bz * 0.008;

      // 3D noise displacement along normal direction
      ...
      if (dist < MOUSE_RADIUS && dist > 5) {
        ...
        const strength = force * force * 50; // Quadratic falloff
        px += (dx / dist) * strength;
        py += (dy / dist) * strength;
        // Also push slightly forward in Z for depth effect
        pz += force * 20;
      }

      // Additional breathing (subtle scale oscillation)
